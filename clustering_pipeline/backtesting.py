...
def model_eval(method, data, train_period, test_period):
    ...
    labels = generate_labels(method, data, train_period, test_period)
...
if __name__ == '__main__':
    ...
    loans_data = mk_env(start_date, end_date, f'backtesting_{method}.log')
    logger = logging.getLogger(__name__)
    
    scores = []
    while not any([np.datetime64(d) > np.datetime64(end_date) for d in (train_period + test_period)]):
        score = model_eval(method, loans_data, train_period, test_period)
        scores.append(score)
        logger.info(
    ...
    logger.info(f'Average Brier Score of {method} is {average_score}.')
